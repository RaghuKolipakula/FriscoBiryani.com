...
export default function ViralQuizPage() {
  ...
  const shareText = finalResult 
    ? `I'm the ${finalResult.title} — what type of biryani eater are you? Find out at friscobiryani.com/quiz!`
    : "";
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: shareText,
          url: "https://friscobiryani.com/quiz",
        });
      } catch (err) {
      ...
      // Fallback
      navigator.clipboard.writeText(`${shareText} https://friscobiryani.com/quiz`);
      setCopied(true);
  ...
  return (
    <main className="min-h-[90vh] flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden bg-slate-50">
      
      {/* Ambient Backgrounds */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-gradient-to-b from-amber-500/10 to-transparent blur-[100px] pointer-events-none" />
      ...
      <div className="w-full max-w-lg mx-auto relative z-10 transition-all duration-500">
        ...
        {step === "intro" && (
          <div className="text-center space-y-8">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-amber-600 text-xs font-bold uppercase tracking-widest shadow-sm">
              <ChefHat className="w-4 h-4" />
              <span>Personality Quiz</span>
            </div>
            
            <h1 className="text-5xl sm:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-4">
              Find the Best <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-orange-600">Frisco Biryani</span>
            </h1>
            ...
            <button 
              ...
              className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white font-black text-xl shadow-lg transition-all hover:-translate-y-1"
            >
              Start Quiz Now
            </button>
          </div>
            ...
            <div className="space-y-3">
              {currentQuestion.answers.map((answer, idx) => (
                <button
                  ...
                  className="w-full p-4 rounded-xl text-left font-bold text-slate-700 bg-white border border-slate-200 hover:border-amber-400 hover:bg-amber-50 transition-all active:scale-[0.98] shadow-sm flex items-center justify-between group"
                >
        ...
        {step === "loading" && (
          <div className="text-center space-y-6 py-12">
            ...
            <h2 className="text-2xl font-bold text-slate-900 animate-pulse">Calculating your Biryani type...</h2>
            <p className="text-slate-500 text-sm">Analyzing spice tolerance and rice preferences.</p>
            
            <div className="mt-8 p-4 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-xs mx-auto">
              <span className="text-[10px] text-amber-500 font-bold uppercase tracking-widest block mb-2">Finding your match</span>
              <p className="text-slate-900 font-mono text-sm truncate animate-pulse">
                {flippingName}
              </p>
            </div>
          </div>
        ...
        {step === "result" && finalResult && (
          <div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-amber-200 shadow-xl relative overflow-hidden mb-6 group">
              ...
              <div className="text-center relative z-10 space-y-4">
                ...
                <h2 className="text-4xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-orange-600 leading-tight pb-2">
                  {finalResult.title}
                </h2>
                <h3 className="text-lg text-slate-800 font-semibold">
                  "{finalResult.subtitle}"
                </h3>
                <p className="text-slate-600 text-sm leading-relaxed max-w-sm mx-auto pt-2 pb-4">
                  {finalResult.description}
                </p>
                
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 text-left mt-4 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-3 opacity-5 group-hover:opacity-10 transition-opacity text-4xl">
                    📍
                  </div>
                  ...
                  {restaurantMatch ? (
                    <>
                      <h4 className="text-slate-900 font-bold text-xl mb-2">{restaurantMatch.name}</h4>
                      <div className="flex items-center gap-2 mb-4">
                        <span className="bg-amber-100 text-amber-800 border border-amber-200 text-xs font-bold px-2 py-0.5 rounded flex items-center gap-1">
                      ...
                      {comparedCount > 1 && (
                        <div className="mb-4">
                          <button 
                            ...
                            className="text-xs text-slate-500 hover:text-amber-600 transition-colors flex items-center gap-1 font-medium"
                          >
                            <ChefHat className="w-3 h-3" />
                            We analyzed {comparedCount} local spots for this match. {showCompared ? "Hide" : "See them"}
                          </button>
                          {showCompared && (
                            <div className="mt-2 p-2 bg-white rounded-lg border border-slate-200 text-[10px] text-slate-600 flex flex-wrap gap-1.5 shadow-inner">
                              {comparedList.map((name, i) => (
                                <span key={i} className="bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                                  {name}
                                </span>
                              ))}
                            </div>
                      ...
                      <p className="text-slate-600 text-sm italic border-l-2 border-slate-300 pl-4 py-1 mb-5 leading-relaxed">
                      ...
                      <a 
                        ...
                        className="inline-flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold transition-all active:scale-[0.98] shadow-md"
                      >
                        View on Google Maps <ArrowRight className="w-4 h-4" />
                      </a>
                    </>
                  ) : (
                    <span className="text-emerald-600 font-bold block">{finalResult.dish}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <button 
                onClick={handleShare}
                className="w-full py-4 rounded-2xl bg-slate-900 text-white font-black text-lg shadow-xl hover:bg-slate-800 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
              >
              ...
              <div className="flex gap-2 w-full mt-2">
                <a 
                  href={`https://wa.me/?text=${encodeURIComponent((finalResult ? `I'm the ${finalResult.title} — what type of biryani eater are you? Find out at ` : '') + 'https://friscobiryani.com/quiz')}`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex-1 py-3 rounded-xl bg-[#25D366] text-white text-sm font-bold flex items-center justify-center hover:bg-[#1DA851] transition-colors shadow-md"
                >
                  WhatsApp
                </a>
                <a 
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(finalResult ? `I'm the ${finalResult.title} — what type of biryani eater are you? Find out at ` : '')}&url=https://friscobiryani.com/quiz`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex-1 py-3 rounded-xl bg-[#1DA1F2] text-white text-sm font-bold flex items-center justify-center hover:bg-[#1a8cd8] transition-colors shadow-md"
                >
                  Twitter
                </a>
                <a 
                  href={`https://www.facebook.com/sharer/sharer.php?u=https://friscobiryani.com/quiz`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex-1 py-3 rounded-xl bg-[#1877F2] text-white text-sm font-bold flex items-center justify-center hover:bg-[#166FE5] transition-colors shadow-md"
                >
                  Facebook
                </a>
              </div>
              ...
              <div className="text-center pt-4">
                <button 
                  onClick={resetQuiz}
                  className="text-slate-500 hover:text-slate-900 text-sm font-bold transition-colors flex items-center justify-center gap-1.5 mx-auto"
                >
